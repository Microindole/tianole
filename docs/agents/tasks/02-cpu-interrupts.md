...
- user/kernel exception policy 接口必须携带 `trap_frame` 和 trap origin，不能只传诊断名称；后续进程终止、signal 和用户栈修复都需要完整现场。
...
- IRQ handler 返回后必须经过明确的 trap-exit 边界，再由调度器消费 pending reschedule；不要让设备 handler 直接执行线程切换。
- 后续 APIC、IOAPIC、MSI/MSI-X 要能替换 PIC 路径，而不改通用 IRQ consumer。

### 5. Trap-exit and return-to-user boundary

- IRQ exit、future syscall return 和 future user exception return 要共享一类
  trap-exit 边界，类似 Linux 在返回用户态前统一处理 pending work 的方向。
- 当前实现中 `x86_trap_exit(frame, origin, reason)` 是这个架构边界：
  IRQ exit 会调用 `sched_irq_exit(frame)` 消费 `need_resched`；syscall 和
  user-exception reason 目前只保留接口，不实现完整返回用户态。
- `sched_irq_exit()` 必须接收 `struct trap_frame *`，即使调度器当前不解释
  frame，也要避免后续 syscall/user-mode return 再重写 API。
- 在正式用户态存在前，不实现 syscall ABI、不引入进程/signal，也不重写 IDT。


### 6. IDT and trap dispatch cleanup
...
- user/kernel 未处理异常 policy 已携带完整 `trap_frame` 和 trap origin，不再只传诊断名称。
...
- 已建立 `x86_trap_exit(frame, origin, reason)`，让 IRQ exit、future syscall return 和 future user exception return 汇合到同一类边界；当前只有 IRQ reason 会实际调用 `sched_irq_exit(frame)` 消费 pending reschedule。
...
- nested interrupt 和更完整的 preempt/reschedule 策略。
