...
| `02-cpu-interrupts.md` | 进行中 | 已有 GDT/TSS/IDT、trap frame、IRQ0/IRQ1；入口表已包含 gate/DPL/IST，page fault、double fault、`#UD`、`#GP` 已有独立 handler。 |
...
优先继续 `02-cpu-interrupts.md`：推进系统向量、用户态异常返回边界和更完整的异常恢复策略。default handler 只保留为未知或暂未覆盖 vector 的兜底，不能继续承载常见异常策略。
