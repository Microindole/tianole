...
当前最合适的下一步仍在 `04-time-scheduler.md` 内：先把 wait queue 条件检查、wakeup、timeout 和锁规则补成不会丢唤醒的模型，再收紧线程生命周期和更明确的 interrupt-exit reschedule。`05-input-events.md` 的键盘驱动应等这些边界稳定后再开始。
